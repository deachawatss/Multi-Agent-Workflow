...
if command -v notify-send &> /dev/null; then
    if [ -n "$DESCRIPTION" ]; then
        notify-send "$AGENT_NAME" "$DESCRIPTION" -i dialog-information
    else
        notify-send "Claude Code" "$AGENT_NAME" -i dialog-information
    fi
fi

# Speech queue - wait for any running espeak-ng to finish
if command -v espeak-ng &> /dev/null; then
    ...
    # Queue the speech with mkdir lock (Linux compatible)
    ...
    (
        ...
        espeak-ng -v "$VOICE" -s "$RATE" "$MESSAGE"
    ) &
fi
